// src/shipGenerator.js

const SHIP_SIZES = [4, 3, 3, 2, 2, 2, 1, 1, 1, 1];

export function generateShips() {
  // Створюємо пусте поле 10x10
  const board = Array(10).fill().map(() => Array(10).fill(0));

  for (const size of SHIP_SIZES) {
    let placed = false;
    while (!placed) {
      const direction = Math.random() > 0.5 ? 'H' : 'V'; // Горизонтально чи вертикально
...
function canPlaceShip(board, x, y, size, direction) {
  ...
  // Перевірка сусідів (щоб кораблі не торкались)
